          ...
          <div className="mt-6 grid gap-5">
            {demandes.map((d) => (
              <article key={d.id} className="grid gap-4 rounded-[2rem] bg-white p-5 shadow-sm ring-1 ring-[#e5d3b3] md:grid-cols-[220px_1fr]">
                <div>{parsePhotos(d.photos).length > 0 ? <div className="grid gap-2"><img src={parsePhotos(d.photos)[0]} alt={d.titre || "Logement"} className="h-44 w-full rounded-2xl object-cover" />{parsePhotos(d.photos).length > 1 && <div className="grid grid-cols-3 gap-2">{parsePhotos(d.photos).slice(1, 4).map((photo) => <img key={photo} src={photo} alt="Photo logement" className="h-16 w-full rounded-xl object-cover" />)}</div>}</div> : <div className="grid h-44 place-items-center rounded-2xl bg-[#f4ead7] text-sm font-bold text-[#7a6446]">Pas de photo</div>}</div>
                <div>
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div><h2 className="text-2xl font-black">{d.titre || "Sans titre"}</h2><p className="text-sm text-[#7a6446]">{d.type_logement} · {d.quartier}, {d.ville}</p></div>
