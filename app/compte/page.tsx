...
export default function ComptePage() {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem("marbnb_account");
      if (raw) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (mode === "creation") {
      ...
      setMessage("Compte créé ✅ Vous pouvez maintenant utiliser Marbnb avec votre profil.");
      return;
    }
  ...
  return (
    <main className="min-h-screen bg-[#f4ead7] px-4 py-8 text-[#1e1b18]">
      <section className="mx-auto max-w-3xl rounded-[2rem] bg-[#fff8ec] p-6 shadow-sm ring-1 ring-[#e5d3b3] md:p-8">
        <p className="font-black text-[#c1121f]">Compte Marbnb</p>
