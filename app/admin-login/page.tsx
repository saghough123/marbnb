...
export default function AdminLoginPage() {
  ...
  function connexion(e: React.FormEvent) {
    e.preventDefault();
    const expected = process.env.NEXT_PUBLIC_ADMIN_PASSWORD || "MbnbAdmin2026";
    if (password === expected) {
      localStorage.setItem("mbnb_admin_ok", "true");
  ...
  return (
    <main className="min-h-screen bg-[#f4ead7] px-4 py-10 text-[#1e1b18]">
      <section className="mx-auto max-w-md rounded-[2rem] bg-[#fff8ec] p-6 shadow-sm ring-1 ring-[#e5d3b3] md:p-8">
        ...
        <p className="mt-6 font-black text-[#c1121f]">Admin Mbnb</p>
        ...
        <div className="mt-5 rounded-2xl bg-[#f4ead7] p-4 text-xs text-[#7a6446]">
          <b>Info :</b> mot de passe par défaut : <b>MbnbAdmin2026</b>. Tu peux le changer dans <b>.env.local</b> et dans Vercel avec <b>NEXT_PUBLIC_ADMIN_PASSWORD</b>.
        </div>
      </section>
    </main>
  );
